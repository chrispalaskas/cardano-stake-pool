...
minFee = 180000 # Fee to withhold from each sender
...
def getRawTx(txInList, initLovelace, initToken, returnAddr, recipientList, ttlSlot, fee, tokenPolicyId):
...
def sendTokenToAddr(myPaymentAddrSignKeyFile, txInList, initLovelace, initToken, 
                    fromAddr, recipientList, tokenPolicyId):
    ...
    print ('Min fee:', fee)
    getRawTx(txInList, initLovelace, initToken, fromAddr, recipientList, ttlSlot, fee, tokenPolicyId)
...
def getCnodeJournal(paymentAddr, tokenPolicyId, myTxHash):
    ...
    # Read systemd journal cnode.service for "Chain extended"
    j = journal.Reader()
    j.log_level(journal.LOG_INFO)

    j.add_match(_SYSTEMD_UNIT="cnode.service")
    j.seek_tail()
    j.get_previous()
    ...
    while p.poll():
        if j.process() != journal.APPEND:
            continue
        for entry in j:
            # Check when new blocks arrive
            if entry['MESSAGE'] != "" and "Chain extended" in entry['MESSAGE']:
